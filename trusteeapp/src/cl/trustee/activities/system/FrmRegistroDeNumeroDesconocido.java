...
public class FrmRegistroDeNumeroDesconocido extends Activity implements
		OnClickListener {
	...
	public void onClick(View view) {
		if (view == btnCancelar) {
			...
			params.add(new BasicNameValuePair("nombre", edtNombre.getText()
					.toString()));
			params.add(new BasicNameValuePair("numero", txtNumero.getText()
					.toString()));
			email = "";
			...
			RadioButton selectRadio = (RadioButton) findViewById(radioEmpresa
					.getCheckedRadioButtonId());

			String selection = selectRadio.getText().toString();
			String empresa = selection.equalsIgnoreCase("EMPRESA") ? "1"
			...
			if (response != null && response.httpResponse != null) {
				int statusCode = response.httpResponse.getStatusLine()
						.getStatusCode();
				if (statusCode == HttpStatus.SC_OK) {
					Toast toast = Toast.makeText(this,
							"Se ha registrado el número", Toast.LENGTH_SHORT);
					toast.show();
				} else if (statusCode == HttpStatus.SC_FORBIDDEN
						|| statusCode == HttpStatus.SC_BAD_REQUEST
						|| statusCode == HttpStatus.SC_UNAUTHORIZED) {
					Toast toast = Toast
							.makeText(this, "No se ha registrado el número",
									Toast.LENGTH_SHORT);
					toast.show();
				} else {
					Toast toast = Toast
	...
	/**
	 * Metodo que abrirá dialogo para indicar que se han realizado multiplo de
	 * 10 colaboraciones.
	 * 
	 * @param context
	 *            El contexto en el que se esta ejecutando.
	 */
	private void notificarDiezColaboraciones() {
		SharedPreferences preferences = PreferenceManager
				.getDefaultSharedPreferences(this);
		int nroTemporalSend = preferences.getInt(
				EPreferences.NRO_TEMPORAL_SEND.toString(), 0);
		nroTemporalSend = nroTemporalSend + 1;

		SharedPreferences.Editor editor = preferences.edit();
		editor.putInt(EPreferences.NRO_TEMPORAL_SEND.toString(),
				nroTemporalSend);
		editor.commit();

		if (nroTemporalSend % 10 == 0) {
			Intent mIntent = new Intent(FrmRegistroDeNumeroDesconocido.this,
					NotificarDiezColaboraciones.class);
