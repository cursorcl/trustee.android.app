...
/**
 * Esta clase permite el ingreso de la informacion posterior a una llamada
 * respondida y validada.
 * 
 * @author cursor
 * 
 */
	...
	private class FeedbackTask extends AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... params) {
			...
			pairValues.add(new BasicNameValuePair("comentario", comment
					.getText().toString()));
			pairValues.add(new BasicNameValuePair("recibir", reciveMail
					.isChecked() ? "1" : "0"));
			email = "";
		...
		protected void onPostExecute(Boolean result) {
			if (result)
				Toast.makeText(CalificaContenidoLlamada.this,
		...
		protected void onCancelled() {
			Toast.makeText(CalificaContenidoLlamada.this, "Tarea cancelada!",
